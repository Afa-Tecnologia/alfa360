          ...
          <Tabs defaultValue="checkout" className="w-full">
            ...
            <TabsContent value="checkout">
                ...
                {cart.map((product) => (
                  <div key={product.id} className="flex items-center justify-between border-b pb-2">
                    <div className="flex items-center space-x-2 w-full">
                      ...
                      <div>
                        <p className="font-medium break-normal">{product.name}</p>
                        <p className="text-sm text-muted-foregroud break-normal">{product.description}</p>
                      </div>
                    </div>
                    ...
                    <div className="font-bold text-sm">R$ {(parseFloat(product.selling_price) * product.quantity).toFixed(2)}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="border-t pt-4">
                <div className="flex justify-between items-center">
                  <span>Total</span>
                  <span className="text-[1.3rem] font-bold">R$ {cart.reduce((acc, item) => acc + parseFloat(item.selling_price) * item.quantity, 0).toFixed(2)}</span>
                </div>
